...
```json
{
	...
	"value": {
		"msg": [{
			"type": "dip/MsgSend",
			"value": {
				"from_address": "dip13f5tmt88z5lkx8p45hv7a327nc0tpjzlwsq35e",
				"to_address": "dip13lmppkumkmf6699q4gpukg8fz5pf2lgzm8mfsm",
				"amount": [{
					"denom": "pdip",
					"amount": "1"
				}]
...
其中```account_number```和```sequence```可通过 [account API接口](../api/api.md#查询帐户信息及余额) 获得， ```account_number```是固定的，```sequence```表示当前帐户的交易数 。
